...
def scrape_amazon_product(url):
    ...
    title = _first_match(page,[r'id="productTitle"[^>]*>(.*?)</span>',r'<meta[^>]+property="og:title"[^>]+content="([^"]+)"',r'<title>(.*?)</title>'])
...
def publish_next():
    with app.app_context():
        ...
        settings=get_settings(); quality_errors=validate_article(settings,dict(item),item["local_image"],item["image_url"],item["product_price"])
        if quality_errors:
        ...
        try:
            ...
            with get_db() as db:
                db.execute("UPDATE articles SET status='published',published_at=?,error='',updated_at=? WHERE id=?",(published_at,published_at,item["id"])); db.execute("DELETE FROM queue WHERE id=?",(item["qid"],))
    ...
    if request.method=="POST":
        ...
        with get_db() as db:
            ...
            if upload and upload.filename:
                suffix=Path(upload.filename).suffix.lower() or ".jpg"; path=UPLOAD_DIR/f"product_{product_id}{suffix}"; upload.save(path); local=str(path)
...
def article_approve(article_id):
    with get_db() as db:
        ...
        if not existing:
            pos=db.execute("SELECT COALESCE(MAX(position),0)+1 p FROM queue").fetchone()["p"]; db.execute("INSERT INTO queue(article_id,position,approved_at) VALUES(?,?,?)",(article_id,pos,now())); db.execute("UPDATE articles SET status='queued',updated_at=? WHERE id=?",(now(),article_id))
...
def article_send_now(article_id):
    ...
    if not article: flash("Articolo non trovato.","error"); return redirect(url_for("index"))
    settings=get_settings(); quality_errors=validate_article(settings,dict(article),article["local_image"],article["image_url"],article["product_price"])
    if quality_errors:
        message="Controllo pre-pubblicazione: "+"; ".join(quality_errors)
...
def drafts_page():
    ...
    items=[]
    for row in rows:
        item=dict(row); item["preview_image"]=url_for("uploads",filename=Path(item["local_image"]).name) if item.get("local_image") else item.get("image_url",""); items.append(item)
...
def settings_page():
    if request.method=="POST":
        allowed=["ai_engine","openai_api_key","openai_model","gemini_api_key","gemini_model","smtp_host","smtp_port","smtp_user","smtp_password","smtp_from","smtp_to","smtp_security","email_subject_prefix","postie_category","amazon_credential_id","amazon_secret","amazon_partner_tag","amazon_marketplace","amazon_token_url","amazon_api_base","publish_days","publish_time","timezone","scheduler_enabled","show_discover"]
        values={key:request.form.get(key,"").strip() for key in allowed}; values["scheduler_enabled"]="1" if request.form.get("scheduler_enabled") else "0"; values["show_discover"]="1" if request.form.get("show_discover") else "0"; set_settings(values); configure_scheduler(get_settings(),publish_next); flash("Impostazioni salvate.","success"); return redirect(url_for("settings_page"))
